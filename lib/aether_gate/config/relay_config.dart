...
class AetherRelayConfig {
  ...
  /// Kept under 3 days so a user who Skipped is re-invited within their
  /// next few sessions. Non-round on purpose (§7a — rotated off template).
  static const int pushSnoozeSeconds = 237600; // 2.75 d
  ...
  /// Real iOS bundle identifier used as `bundle_id` in the config POST body
  /// and any partner-facing identity that MUST match App Store Connect /
  /// Xcode. Distinct from `uaBundleValue` (which encodes the numeric-id
  /// token used only inside the User-Agent, e.g. `appid/id6797957269`).
  static String get bundleId => BoltCipher.reveal(_bundleId);
  ...
  static const List<int> _uaBundleValue = <int>[0x08, 0xD9, 0x12, 0x44, 0x77, 0xC2, 0xD4, 0xC7, 0x3D, 0xE2, 0xAA, 0xDB];
  static const List<int> _uaAppNameValue = <int>[0x23, 0xD2, 0x48, 0x07, 0x01, 0x93, 0xAC, 0x97, 0x7E, 0xB8, 0xF9, 0x90];
  static const List<int> _bundleId = <int>[0x02, 0xD2, 0x49, 0x5D, 0x2C, 0x9A, 0x81, 0x86, 0x6B, 0xB5, 0xE8, 0x8A, 0xBA, 0x3D, 0x29, 0x40, 0xCA, 0x71, 0x37, 0x25, 0x93, 0x43, 0x93, 0xB2, 0xED, 0xA9, 0x9C, 0x8A, 0x3D];
